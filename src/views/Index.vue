  ...
  <v-container fill-height >
    ...
    <v-row justify="center" align="center">
      <v-card class="card" :loading="isAjax">
        ...
        <v-card-title>
          <a target="_blank" href="https://explorer.avax.network/asset/uG1V6JkLi8APvFJL9YoZSFNb5PdrWAdpePhVWPPBt5FWnF75h">AFV NFT Faucet</a>
        </v-card-title>
        <v-card-subtitle>
          <a target="_blank" href="https://twitter.com/">preview the NFT</a>
        </v-card-subtitle>

        <v-card-text v-show="state==='form'">
          <div>
            <label>Address (Enter your P-address)</label>
          ...
          <v-alert type="error" dense outlined>
            Only the 1000 FIRST validators can ask ONE time this NFT.
          </v-alert>
          <v-btn class="submit" @click="onSubmit" block :loading="isAjax" depressed :disabled="!canSubmit">REQUEST MY VALIDATOR NFT</v-btn>
        </v-card-text>
      ...
      <v-card-text>from <a target="_blank" href="https://github.com/ava-labs/avalanche-faucet">Avalanche faucet</a>, by <a target="_blank" href="https://twitter.com/qsvtr/">@qsvtr</a> with ❤️</v-card-text>
    </v-row>
  </v-container>
</template>
<script>
  ...
  methods:{
    ...
    onresponse(res){
      ...
      if(data.status === 'success'){
        ...
        this.state = 'info'
        this.responseInfo = data.message
      }else{
